...
Use `styleName` to connect a component to a local CSSX class:
...
`styleName` works like `className` in HTML, but it maps to CSSX classes. You
can pass a string, an array, or an object for conditional styles:
