...
- **Modal**: the API of the Modal component has changed. [See more](https://startupjs-ui.dmapper.co/docs/components/Modal)
  ...
  - add `showCross`, `onCrossPress`, `enableBackdropPress`, `supportedOrientations`, `$visible`, `onOrientationChange`
  - Previously, `Modal` required `useState`, but now you need to use `useValue` and pass the `$visible` setter property instead of `visible`
