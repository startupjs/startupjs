...
- **@dmapper/permissions**: There was changed parameters of functions in `hasPermission` and `hasParentPermission`. Now they look like this:

  ```js
  hasPermission(permission, model, collection, docId, doc, session)
  //
  hasParentPermission(permission, model, collection, docId, doc, session)
  ```
