...
4. Use `StartupjsProvider theme='auto'` only when your app is ready for automatic light/dark selection.
...
`:root` theme. This keeps fresh Expo Router / React Navigation apps from
rendering a light navigation shell around dark CSSX components.
...
`useTheme()` or `DarkMode` takes priority after the user chooses one.

Use:

- no `theme` prop for the default root preference with saved user preference support
- `theme='auto'` to select `dark` on dark systems when `:root.dark` exists
