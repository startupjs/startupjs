...
build_image () {
  ...
  then
    ...
    then
      ...
      docker build -t "${REGISTRY_SERVER}/${APP}:${COMMIT_SHA}" -f "$DOCKERFILE_PATH" /_project
      docker tag "${REGISTRY_SERVER}/${APP}:${COMMIT_SHA}" "${REGISTRY_SERVER}/${APP}:latest"
      docker push "${REGISTRY_SERVER}/${APP}:${COMMIT_SHA}"
      docker push "${REGISTRY_SERVER}/${APP}:latest"
      # executor \
      #   --context /_project \
      #   --dockerfile "$DOCKERFILE_PATH" \
