...
- `onCrossPress` callback that is executed when you click on the cross in the header (it has a higher priority than `onClose`)
- `onClose` displays cancel button that uses this handler
...
Modal consists of three parts - `Header`, `Content` and `Actions`. These parts can be used to add custom markup, the `Header` is used instead of `title`, the `Content` is used instead of children and the `Actions` is used instead of handlers `onClose`, `onConfirm`. They can be used separately.
