...
You don't need to create your `useValue` and pass its setter to the modal. To control the state of the modal, you can create a variable with `const modalRef = useRef()` and pass it to the modal `ref={modalRef}`, and then call the `open` and` close` methods of `modalRef.current` to control modalk state.
