...
- `onBackdropPress` callback, which is executed when clicking on the area around the modal
...
### There are two options for managing the state of a modal:
  1. `useValue` - you create `useValue` and pass its setter to the `$visible` parameter and then to close the modal you need to change the scope model in the callbacks that are passed to the modal for various actions (`$visible.set(false)`).
  2. `useRef` - you create `useRef()` and pass it to the modal in the ref parameter and then you can call `modalRef.current.open()` and `modalRef.current.close()` to control the state of the modal. In this case, it is not necessary to call the closure of the modal in the `onConfirm` and `onCancel` callbacks, the closure will be triggered automatically.
