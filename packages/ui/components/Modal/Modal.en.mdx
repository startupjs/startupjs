...
### There are two options for managing the state of a modal:
  1. Having passed the scope-model to the modal via the `$visible` parameter.
  2. By passing `ref` to the modal, which will receive the `open()` and `close()` methods that you will call to manage the modal state.
