...
### Для управления состоянием модалки есть два варианта:
  1. Передав scope-модель в модалку через параметр `$visible` и тогда для закрытия модалки необходимо изменять scope-модель в колбеках, которые передаются в модалку на различные действия (`$model.set(false)`).
	2. Передав `ref` в модалку, который получит методы `open()` и `close()`, которые вы будете вызывать для управления состоянием модалки. В этом случае необязательно в колбеках `onConfirm` и `onCancel` самим вызывать закрытие модалки, закрытие будет срабатывать автоматически.
