...
### Для управления состоянием модалки есть два варианта:
  1. Передав scope-модель в модалку через параметр `$visible`.
	2. Передав `ref` в модалку, который получит методы `open()` и `close()`, которые вы будете вызывать для управления состоянием модалки.
