...
```jsx example
const blocktyle = { width: 56, marginRight: 8, marginRight: 8, marginTop: 4, marginBottom: 4 }
const contentStyle = { paddingLeft: 24, paddingRight: 24, paddingTop: 16, paddingBottom: 16 }
...
function getPopover(placement, index) {
  if (!placement) return <View key={index} style={blocktyle} />
  ...
  return (
    ...
    >
      <Popover.Caption style={blocktyle}>
