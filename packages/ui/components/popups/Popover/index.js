...
const Popover = ({
  ...
  const [opacityOverlay] = useState(new Animated.Value(visible ? 1 : 0))
  ...
  const show = contentHeight => {
    const animated = () => {
      Animated.parallel([
        ...
        Animated.timing(opacityOverlay, { toValue: 0.5, duration: 300 }),
  ...
  const hide = () => {
    Animated.parallel([
      ...
      Animated.timing(opacityOverlay, { toValue: 0, duration: 400 }),
  ...
  const _styleOverlay = { ...styleOverlay, opacity: opacityOverlay }
