...
Бейджи бывают двух видов: `default` и `dot`. `default` является наиболее часто используемым и определен по умолчанию. `dot` превращает значок в маленькую точку, это можно использовать как уведомление о том, что что-то изменилось без контента или как индикатор изменения состояния. При значении свойства `variant='dot'` свойство `size` будет игнорироваться.
...
Если свойство `label` содержит числовое значение, то можно задать свойство `max` чтобы ограничить максимальное отображаемое число в лейбле.
