import Badge from '../Badge'
import Row from '../Row'
import Button from '../Button'
import Div from '../Div'
import Icon from '../Icon'
import { faThumbsUp, faEnvelope } from '@fortawesome/free-solid-svg-icons'
import { Sandbox } from '@startupjs/docs'

# Badge

Badge is small state descriptors for UI elements.

```js
import { Badge } from '@startupjs/ui'
```

## Simple exmaple

```jsx example
return (
  <Badge>
    <Icon icon={faEnvelope} size={40} />
  </Badge>
)
```

## Sizes

Badges come in three different sizes: `s`, `m`, and `l`. The `m` size is the default and most frequently used. The other sizes should be used with care so as not to break the hierarchy of importance on the page.

```jsx example
return (
  <Row>
    <Badge label='16' size='s'>
      <Div pushed><Icon icon={faEnvelope} size={40} /></Div>
    </Badge>
    <Badge label='20'>
      <Div pushed><Icon icon={faEnvelope} size={40} /></Div>
    </Badge>
    <Badge label='24' size='l'>
      <Div pushed><Icon icon={faEnvelope} size={40} /></Div>
    </Badge>
  </Row>
)
```

## Variants

Badges are of two types: `default` and `dot`. `default` it is the most frequently used and the default specified. `dot` turns the icon into a small dot, this can be used as a message that something has changed without content or as an indicator of a state change. If the property is `variant='dot'`, the `size` property will be ignored.

```jsx example
return (
  <Row>
    <Badge label='10'>
      <Div pushed><Icon icon={faEnvelope} size={40} /></Div>
    </Badge>
    <Badge variant='dot'>
      <Div pushed><Icon icon={faEnvelope} size={40} /></Div>
    </Badge>
  </Row>
)
```

## Colors

Color is `primary` by default. It can be changed by passing color name from the config colors to `color` property. Possible values of property can be found in the `Sandbox` section at the bottom of the page.

```jsx example
return (
  <Badge color='error'>
    <Div styleName='child' pushed/>
  </Badge>
)
```

## Position

Badge can be in two positions: `top` and `bottom`. Default value is: `top`.

```jsx example
return (
  <Row>
    <Badge>
      <Div pushed><Icon icon={faEnvelope} size={40} /></Div>
    </Badge>
    <Badge position='bottom'>
      <Div pushed><Icon icon={faEnvelope} size={40} /></Div>
    </Badge>
  </Row>
)
```

## Icons

Icon can be added using `icon` property.

```jsx example
return (
  <Badge icon={faThumbsUp} label='10+' color='success'>
    <Icon icon={faEnvelope} size={40} />
  </Badge>
)
```

## Counter

If `label` property takes a numeric value, then the `max` property can be set to limit the maximum displayed number in the label.

```jsx example
return (
  <Badge label={1000} max={100}>
    <Icon icon={faEnvelope} size={40} />
  </Badge>
)
```

## Sandbox

<Sandbox Component={Badge} />
