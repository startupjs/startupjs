...
Цвет кнопки можно задать указав свойство `color` c именем цвета из $UI.colors.
Default `color` = 'dark'.

Цвет иконки можно задать указав имя цвета из $UI.colors.

Свойство `variant` отвечает за внешнее отображение кнопки(влияет на цвет иконки и текста кнопки). Default `variant` = 'outlined'.
