...
The color for button can be set by specifying the `color` property with the color name from $ UI.colors.
Default `color` = 'dark'.

The icon color can be set by specifying the color name from $ UI.colors.

The `variant` property is responsible for the external display of the button (affects the color of the icon and button text). Default `variant` = 'outlined'.
