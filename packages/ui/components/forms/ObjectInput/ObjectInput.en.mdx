...
ObjectInput accepts an object in the `properties` property with a required `input` key to specify what type of input to display. Possible types are ([сheckbox](/docs/en/components/Checkbox), [text](/docs/en/components/TextInput) & [object](/docs/en/components/ObjectInput)). The `properties` object also inherit any of the props passed to the these components.
