...
Если передан колбек `onClose` и не задано свойство `renderActions`, отображается значок закрытия по умолчанию.
...
Свойство `renderActions` можно использовать для предоставления альтернативного компонента, например, с помощью `Button`, `Tag` или `Icon`.
