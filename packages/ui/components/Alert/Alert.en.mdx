...
If an `onClose` callback is provided and no `renderActions` prop is set, a close icon is displayed.
...
The `renderActions` prop can be used to provide an alternative action, for example using a `Button`, `Tag` or `Icon`.
