...
The ability to scroll with the mouse or finger is set by isSwipe (default is ture)
...
If you need full customization of arrows or points, can be transferred ref, it will contain functions to switch: toLeft, toRight, toIndex
...
return (
  <Div style={{ overflow: 'hidden' }}>
    <Row style={{ marginBottom: 4 }}>
      <Div style={arrowStyle} onPress={()=> ref.current.toLeft()}>
        <Text style={{ color: '#fff' }}>{'<'}</Text>
      </Div>
      <Div style={arrowStyle} onPress={()=> ref.current.toRight()}>
        <Text style={{ color: '#fff' }}>{'>'}</Text>
      </Div>
    </Row>
    <Carousel
