...
**Important:** Portal.Provider should not be located above the components associated with react-router, since some UI components working in conjunction with Portal use its API (eg: useHistory in Dropdown). So it's best to wrap a separate Layout, not the whole app in Root/App
