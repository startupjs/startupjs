...
`H*` комопненты используются для заголовок или подзаголовков раздела или страницы. Основное отличие от компонента `Span` с миксином `font(size)` заключается в том, что `H*` вставляют семантические теги в DOM на вебе.
