...
## Tooltip
- renderTooltip (function: ReactNode | string | number): render function which must return the content of the tolltip (JSX, string or number)
- tooltipProps (object): additional parameters identical to [Popover](/docs/popups/Popover): style, position, attachment, durationOpen, durationClose, arrow

```jsx example
const content = 'Content'

return pug`
  Div.caption(renderTooltip=content)
