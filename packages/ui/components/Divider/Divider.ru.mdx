...
export function SandboxEvents () {
  useEffect(()=> {
    $root.scope('_session.Props.Divider.variant').on('change', variant=> {
      if (variant === 'horizontal') {
        $root.set('_session.Props.Divider.style', { width: '100%' })
      } else{
        $root.set('_session.Props.Divider.style', { height: 48 })
      }
    })
  }, [])
  return null
