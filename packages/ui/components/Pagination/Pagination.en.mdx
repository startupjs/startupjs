...
import { useValue, useSession } from 'startupjs'
...
## Two-way data bindings двустороннию привязку данных

Properties `page`, `limit` and `skip` have two-way data bindings `$page`, `$limit` and `$skip` respectively.
