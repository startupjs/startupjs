...
For e2e testing on mobile platforms, Styleguide uses the [Detox](https://github.com/wix/Detox) library  and test-runner [Jest](https://jestjs.io/)

## Running tests

To run Styleguide testing locally, you need to run the following commands:
- `yarn test: build: ios` - builds the iOS application (required for the first time and after installing new packages)
...
At the root of the component folder, create a file `* .e2e.js` and add a minimal template to it:
