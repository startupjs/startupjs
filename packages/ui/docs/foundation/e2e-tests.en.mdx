...
### Creating a Snapshot
1. Set the `NO_MONGO` flag to `true`.
2. In the config, specify `DB_PATH` name for the snapshot - `test.db`.
3. Start the application and create a snapshot for tests.

### Creating a Test
4. Copy the snapshot into the test folder.
5. Start the test generator using the command:
 ```bash
NO_MONGO=true DB_READONLY=true DB_LOAD_SNAPSHOT=test/snapshots/test.db startupjs codegen-test-e2e
   ```
6. Save the generated test in the E2E tests folder.
