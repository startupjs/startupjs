...
### Создание снапшота
1. Установите флаг `NO_MONGO` в `true`.
2. В конфиге укажите `DB_PATH` имя для снапшота - `test.db`.
3. Запустите приложение и создайте снапшот для тестов.

### Создание теста
4. Скопируйте снапшот в папку тестов.
5. Запустите генератор тестов с помощью команды:
    ```bash
   NO_MONGO=true DB_READONLY=true DB_LOAD_SNAPSHOT=test/snapshots/test.db startupjs codegen-test-e2e
   ```
6. Сохраните сгенерированный тест в папку E2E тестов.

### Запуск теста
7. Запустите тесты с помощью команды:
```bash
  NO_MONGO=true DB_READONLY=true DB_LOAD_SNAPSHOT=test/snapshots/test.db TEST_E2E_DIR=test/e2e startupjs test-e2e
```
