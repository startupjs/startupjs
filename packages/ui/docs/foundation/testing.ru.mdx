...
После первого запуска теста создаcтся скриншот в папке `e2e/__image_snapshots__`. Если при последующем запуске теста, скриншоты не будут совпадать, то в папке `e2e/__diff_output__` создаcтся файл с отображением различий между первоначальным скриншотом и текущим состоянием теста.
