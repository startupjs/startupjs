...
Пример изоморфной валидации для показа страницы админки, который отработает и при клиентском переходе на страницу и при серверном:
...
Пример валидации POST запросов REST API роутов на сервере:
...
// массив с обработчиками, где первым будет мидлвара с проверкой.
adminRouter.post('/api/add-admin-task', [isAdmin, addAdminTask])
