...
2. Use [Racer](https://derbyjs.com/docs/derby-0.10/models) to add a model to your application to perform any data manipulation
3.	The model works like a global singleton state, so you can use it as a replacement for other state managers like Redux or Mobx.
4. Make the component render reactive, as it is in Mobx, whenever any of the model data that you used in the render is altered.
...
`docc` [Array] -- an array of documents

`$docc` [Model] -- a model that points to `collection`
...
```jsx
{
  reverse: true // will change the order of documents in the resulting array
}
```
