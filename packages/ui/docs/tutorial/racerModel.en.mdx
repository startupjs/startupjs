...
You have already worked with model in the chapter on creating the Todo List app. Model is a core concept in Startupjs. With its help, we perform each of the CRUD operations on the document and add documents to the collection. For the client side, Racer provides ORM (Object-Relational Mapping). In simple terms, ORM is a system that provides functionality for working with a database in the form of object methods. That is, using ORM you have an object responsible for a specific document or collection. This object has methods for each of the CRUD operations.
...
Now let's try to understand what a model is. There will be a bit of abstraction. Conceive an imaginary mark that you can put on the desired object in the city. For example, you want to tag a block or a specific house in a block, or even a specific person. With this tag you can quickly find the component you want. The model plays a role of such a label. To put such a label is called “to subscribe”. 

Now when we have a model of an object, we can interact with it. The type of interaction depends on the type of this object. This is obvious, you cannot, for example, add a new house to the model of a person, since this makes no sense. But you can add it to a block. 
...
1. 1.	Take the selected scope for the model (called scope).

```js
  const $todos = model.scope('todos')
```

2. 2.	Now you need to subscribe, note that this subscription returns a Promise.
...
3. 3.	After subscribing, you can perform the necessary interactions with the model.
...
Imagine this method iterates over each element of the object and executes a `set` method, where the first parameter is the element's key and the second is the element's value. I.e:
...
2.There is a `users` collection. Implement the `addUser`, `removeUser` functions.
...
3.	Now let's implement a simple counter example to practice the number-related model methods. Write functions addCounter, reduceCounter. Since this is the 3rd task, let's also implement the addAsync function, which will increment the counter by 1 in 2 seconds after clicking.
