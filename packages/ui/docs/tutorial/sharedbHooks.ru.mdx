...
2. Используют [Racer](https://derbyjs.com/docs/derby-0.10/models) для добавления модели в ваше приложение для выполнения любых манипуляций с данными
3. Модель работает как глобальный синглтон стейт, поэтому вы можете использовать её как замену для других стейт-менеджеров, таких как Redux или Mobx;
4. Делают рендер компонента реактивным, как это делает в Mobx, когда изменяются какие-либо данные модели, которые вы использовали в рендере.
...
`docId` [String] -- id документа. Обязательный
...
> **ВАЖНО:** Id документа в Mongo хранится внутри поля `_id`. Но когда он попадает в модель, он заменяется полем `id`, и наоборот
...
`query` [Object] -- запрос(поддерживаются регулярные выражение, `$count` и `$aggregate`). Обязательный

#### Возвращает:
`[doc, $doc]`, где:

`docc` [Array] -- массив документов

`$docc` [Model] -- модель, которая указывает на `collection`
...
```jsx
{
  reverse: true // поменяет порядок документов в результирующем массиве
}
```
...
`$limit: 1` и `$sort: { createdAt: -1 }` добавляются в `query` автоматически (если не были переопределены)
...
Также можно подписаться на путь из общедоступной коллекции, например, если вы хотите работать с некотороым вложенным значением определенного документа, на который уже подписывались.
