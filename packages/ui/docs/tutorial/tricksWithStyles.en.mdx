...
StartupJS/UI initially uses an 8-pixel grid for padding. This means that the unit of measurement for padding is unit (u). One unit equals 8px. All styles that could be specified in pixels are specified in units.
...
Миксин `shadow()` принимает в себя себя число от `0` до `4`, по умолчанию `1` и разворачивает в селектор набор стилей `box-shadow` по следующей схеме:

The `shadow()` mixin takes a number from `0` to `4`, `1` by default, and extends the selector with the `box-shadow` set of styles in this way:
...
> You can also add your own values or alter the existing ones. This can be done in the `styles/helpers.styl` file.
...
Such transformations of `$UI` object should be performed in the main style sheet file of the project, which you can find at `styles/index.styl`. Since this file will be triggered when the entire project is launched, you will get a modified `$UI` in any style sheet file.
