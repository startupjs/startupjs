...
Let us look at the [documentation for the Content component](https://startupjs-ui.dmapper.co/docs/components/Content#Vertical%20paddings). We see that to add padding on all four sides, we need to add the `padding` prop to component.
...
As with regular HTML, you can use the `style` attribute on a component. Let us add some styling to the `<H1>` element:
...
> `align='between' vAlign='center'` – props from [Row component documentation](https://startupjs-ui.dmapper.co/docs/components/Row), analogue `justify-content: space-between; align-items: center;`
...
According to the [documentation](https://startupjs-ui.dmapper.co/docs/forms/Checkbox), the checkbox has two main props: `value` and `onChange`.
...
> You can learn more about these and other props of the `Button` component in the [documentation](https://startupjs-ui.dmapper.co/docs/components/Button)
...
According to the [documentation](https://startupjs-ui.dmapper.co/docs/forms/TextInput), we need two props from TextInput for our purposes: `value` and `onChangeText`.
