...
It is recommended to use the Pug preprocessor in StartupJS instead of the native JSX syntax. 
...
Pug's syntax is relatively simple. It has no angle brackets or closing tags. The nesting of elements within each other is determined by indentation, using either Tab or space.

> StartupJS implies the use of Tabs for indentation
...
*	Nesting is determined via indentation (Tab or space)
