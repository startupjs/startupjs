...
- `options [Object]` - объект опций. Содержит список провайдеров:
  - `providers [Array]` - массив обьектов провайдеров.

### checkToken (model, session, { data = {}, providerName = '', token = '' })

Функция для проверки кода на сервере. Параметры:
...
### ProvidersBlock(providerNames, onSubmit)

Компонент реализует всю последовательность для двух факторной проверки. Принимает следующие пропсы:
