...
export default function onBeforeRegister (req, res, next, opts) {
  ...
  if (!email) return next('Please fill email')
  ...
  if (password.length < 8) return next('Password length should be at least 8')
  if (!EMAIL_REGEXP.test(email)) return next('Incorrect email')
  if (emailRegistrationRegexp && !emailRegistrationRegexp.test(email)) return next("You can't use this email")
