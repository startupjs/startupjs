...
import { OneSignal, sendNotification, sendAdvancedNotification } from '@startupjs/push-notifications'
...
- `sendNotification({ userIds, title, content })` - функция для отправки нотификаций. Опции:
  - `userIds` - массив id юзеров которым будет отправлено push уведомление. Здесь используются локальные id юзеров из БД. !ВАЖНО! в этой функции отправка происходит по `external_user_id`, (пример работы с `externalUserId` смотрите ниже). 
  - `title` - строка заголовка.
  - `content` - строка контента.

- `sendAdvancedNotification(data)` - функция для отправки нотификаций, используйте структуру запроса как указано в [SERVER REST API](https://documentation.onesignal.com/reference/create-notification). !ВАЖНО! поле `app_id` указывать не нужно.
...
Иcпользуйте данный метод в удобном месте вашего приложения.

## Персонализация уведомлений

В уведомлениях можно использовать уникальные переменные для каждого пользователя. Подробную информацию о пользовательских переменных можно найти [здесь](https://documentation.onesignal.com/docs/data-tag-implementation). Информация об отправке персонализированых сообщений находится [здесь](https://documentation.onesignal.com/docs/personalization).

Присвоение персональных переменных.
...
```js
const personalMessage = "Hello {{ firstName | default: 'there'}}! We just improved our controls, come back and see if you can beat level {{ currentLevel | default: '1' }}!"
...
function sendAdvancedCustomNotification () {
...
return (
  <Div>
    ...
    <Button onPress={sendAdvancedCustomNotification}>
      Send Advanced Test Push
    </Button>
  </Div>
)
```
