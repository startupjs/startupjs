...
Call `initPushNotifications` and `initAndroidChannel` in the place where you need to initialize the device token of the current user (devices are written based on the `userId` from the current session). It makes sense to perform initialization only for an authorized user. But if necessary, initialization is allowed for each visitor, for this functions can be called directly in the `useGlobalInit` callback of `App`.
...
- `initPushNotifications (options)` - function of initializing push notifications.
