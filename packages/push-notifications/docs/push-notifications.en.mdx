...
- `initOneSignal(useInit)` - `useInit` - the function that will be called after registering the application, here you can place functions from the [React Native SDK](https://documentation.onesignal.com/docs/react-native-sdk).
...
- `sendNotification({ userIds, title, content })` - function for sending notifications. Options:
  - `userIds` - an array of user ids to which the push notification will be sent. It uses local user ids from the database. !IMPORTANT! in this function, sending is done by `external_user_id`, (see below for an example of working with` externalUserId`).
  - `title` - string with title.
  - `content` - string with content.

- `sendAdvancedNotification(data)` - function to send notifications, use the request structure as specified in [SERVER REST API](https://documentation.onesignal.com/reference/create-notification). !IMPORTANT! the `app_id` field is not required.
...
Use this method at a convenient location in your application.

## Personalize notifications

You can use unique variables for each user in notifications. More information about custom variables can be found [here](https://documentation.onesignal.com/docs/data-tag-implementation). Information about sending personalized messages is located [here](https://documentation.onesignal.com/docs/personalization).

Assigning personal variables.
...
Send a notification to the current user. To do this, you can use the [sendSelfNotification method](https://documentation.onesignal.com/docs/web-push-sdk#sendselfnotification). This method is only available on the web.
