...
describe('READ', function () {
  before(async () => {
    backend.allowCreate('tasksRead', async (operation, backend, collection, docId, doc, session) => {
      return true
    })
  ...
  it('deny = false && allow = true => not err', async () => {
    backend.denyRead('tasksRead', async (operation, backend, collection, docId, doc, session) => {
      return false
    })
    backend.allowRead('tasksRead', async (operation, backend, collection, docId, doc, session) => {
      return true
    })
