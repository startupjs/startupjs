...
export function ExampleComponent () {
  ...
  useEffect(() => {
    pluginHook('say', { username: user.name })
  }, [])
...
- `func` - метод который после вызова должен вернуть объект с ключами для слотов и хуков. Ключи будут использованы либо в слотах с такими же именим либо при вызове хуков с такими именами
...
export default {
  ...
  func: options => {
    return {
      say: () => console.log(`Hello, ${options.userName}`)
    }
  }
}
```
