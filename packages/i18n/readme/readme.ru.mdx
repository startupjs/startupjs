...
Чтобы не приходилось писать конфиг с переводами вручную можно использовать функцию - `t`
...
Первый аргумент - название тэга для слов, второй значение по умолчанию

При компиляции проекта будут найдены все файлы которые используют данную функцию, и сгенерируется конфиг в формате:
...
Т.е. изначально заполняются слова только для дефолтного языка, остальные слова других языков можно изменять на странице - ${BASE_URL}/i18n
