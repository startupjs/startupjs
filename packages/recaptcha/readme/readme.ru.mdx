...
Для использования [reCAPTCHA Enterprise](https://cloud.google.com/recaptcha-enterprise) необходимо выполнить следующие действия:
...
Использование reCAPTCHA Enterprise на клиенте отличается от использования reCAPTCHA v2 только тем, что callback `onVerify` компонента `Recaptcha` возвращает объект `{ token: 'token', variant: 'normal of visible' }`
